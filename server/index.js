...
//         if (socket.data.name) totalStudents--;
//     });
// });
...
io.on('connection', (socket) => {
    ...
    //     // Find if the disconnected user was a participant
    //     const wasParticipant = participants.find(p => p.id === socket.id);

    //     if (wasParticipant) {
    //         // Remove from participants list
    //         participants = participants.filter(p => p.id !== socket.id);
    //         // Emit updated participants list to everyone
    //         io.emit('update-participants', participants.map(p => p.name));
    //         console.log(`${wasParticipant.name} left.`);
    //     }
    // });
